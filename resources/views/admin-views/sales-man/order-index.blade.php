...
<div class="content container-fluid">
            ...
            <div class="media-body">
                <div class="row">
                    <div class="col-lg mb-3 mb-lg-0 {{Session::get('direction') === "rtl" ? 'mr-2' : 'ml-2'}}" style="display: block; text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                        <div>
                            <h1 class="page-header-title">{{\App\CPU\translate('Sales Team ( Order Report )')}}</h1>
                        </div>
    ...
    <div class="row border-bottom border-right border-left border-top">
        <div class="col-lg-12">
            <form action="{{route('admin.sales-man.team-set-date')}}" method="post">
                @csrf
                <div class="row">
                    <div class="col-12">
                        <div class="mb-3">
                    ...
                    <div class="col-3">
                        <div class="mb-3">
                            <input type="date" value="{{date('Y-m-d',strtotime(session('from_date')))}}" name="from" id="from_date" class="form-control" required>
                        </div>
                    </div>
                    <div class="col-3">
                        <div class="mb-3">
                            <input type="date" name="to" value="{{date('Y-m-d',strtotime(session('to_date')))}}" id="to_date" class="form-control" required>
                        </div>
                    </div>
                    ...
                    <div class="col-2">
                        <div class="mb-3">
                            <select name="team_char" class="form-control @error('group') is-invalid @enderror" required>
                                @for ($i=0;$i<count($chars);$i++) <option value="{{$chars[$i]}}" {{ session('team') == $chars[$i] ? 'selected' : '' }}>Team &nbsp;{{$chars[$i]}}</option>
                                    @endfor
                            </select>
                        </div>
                    </div>
        ...
        $to = session('to_date');
        $team = session('team');
        $selerIds=\App\Model\SalerTeam::where('team',$team)->get(['saler_id']);
        $total=\App\Model\Order::where('order_type','default_type')
        ...
        <div class="col-sm-6 col-lg-3 mb-3 mb-lg-6">
            @php
            $delivered=\App\Model\Order::where('order_type','default_type')
            ->where(['order_status'=>'delivered','customer_type'=>'salesman'])
            ->whereIn('customer_id', $selerIds)
            ->whereBetween('created_at', [$from, $to])
            ->count()
            @endphp
        ...
        <div class="col-sm-6 col-lg-3 mb-3 mb-lg-6">
            @php
            $returned=\App\Model\Order::where('order_type','default_type')
            ->where(['order_status'=>'returned','customer_type'=>'salesman'])
            ->whereIn('customer_id', $selerIds)
            ->whereBetween('created_at', [$from, $to])
            ->count()
            @endphp
        ...
        <div class="col-sm-6 col-lg-3 mb-3 mb-lg-6">
            @php
            $failed=\App\Model\Order::where('order_type','default_type')
            ->where(['order_status'=>'failed','customer_type'=>'salesman'])
            ->whereIn('customer_id', $selerIds)
            ->whereBetween('created_at', [$from, $to])
            ->count()
            @endphp
        ...
        <div class="col-sm-6 col-lg-3 mb-3 mb-lg-6">
            @php
            $canceled=\App\Model\Order::where('order_type','default_type')
            ->where(['order_status'=>'processing','customer_type'=>'salesman'])
            ->whereIn('customer_id', $selerIds)
            ->whereBetween('created_at', [$from, $to])
            ->count()
            @endphp
    ...
    <div class="card mb-3 mb-lg-5 border-bottom border-right border-left border-top">
        ...
        <div class="card-header">
            ...
            $priceNew=\App\Model\Order::where('order_type','default_type')
            ->where(['order_status'=>'delivered','customer_type'=>'salesman'])
            ->whereIn('customer_id', $selerIds)
            ->whereBetween('created_at', [$from, $to])
            ->sum('order_amount');
            ...
            <div class="flex-start">
                <h6 class="card-subtitle mt-1">Total Price of {{date('Y')}} : </h6>
                <h6 class="h3 {{Session::get('direction') === "rtl" ? 'mr-sm-2' : 'ml-sm-2'}}">{{$priceNew}}</h6>
            </div>
        ...
        for ($inc = 1; $inc <= 12; $inc++) { $delivered[$inc]=0; foreach ($data as $match) { if ($match['month']==$inc) { $delivered[$inc]=$match['count']; } } } @endphp @php $ret=[]; $from=\Carbon\Carbon::now()->startOfYear()->format('Y-m-d');
            $to = \Carbon\Carbon::now()->endOfYear()->format('Y-m-d');
            ...
            for ($inc = 1; $inc <= 12; $inc++) { $ret[$inc]=0; foreach ($data as $match) { if ($match['month']==$inc) { $ret[$inc]=$match['count']; } } } @endphp @php $fai=[]; $from=\Carbon\Carbon::now()->startOfYear()->format('Y-m-d');
                $to = \Carbon\Carbon::now()->endOfYear()->format('Y-m-d');
                ...
                for ($inc = 1; $inc <= 12; $inc++) { $fai[$inc]=0; foreach ($data as $match) { if ($match['month']==$inc) { $fai[$inc]=$match['count']; } } } @endphp @php $can=[]; $from=\Carbon\Carbon::now()->startOfYear()->format('Y-m-d');
                    $to = \Carbon\Carbon::now()->endOfYear()->format('Y-m-d');
                    ...
                    for ($inc = 1; $inc <= 12; $inc++) { $can[$inc]=0; foreach ($data as $match) { if ($match['month']==$inc) { $can[$inc]=$match['count']; } } } $max_order=\App\CPU\BackEndHelper::max_orders(); @endphp <!-- Body -->
                        <div class="card-body">
                            <!-- Bar Chart -->
